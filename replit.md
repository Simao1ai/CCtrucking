...
- **AI Assistant (Enhanced)**: Dual-role AI assistant serving as both an internal operations tool AND a trucking industry expert. Includes deep knowledge of FMCSA, DOT, IFTA, UCR, IRS regulations, compliance deadlines, required forms, and government website links. Staff can ask complex regulatory questions and get detailed guidance with citations. Each AI response has a "Save to Documents" button that lets staff save research/guidance as a document linked to a specific client. System prompt in `server/routes.ts` includes live business data + comprehensive trucking industry knowledge base. AI also references internal Knowledge Base articles and cites them when relevant.
- **Knowledge Base**: Internal knowledge library for staff. Owner/admin can create, edit, and pin articles organized by category (Onboarding, IFTA Filing, DOT Compliance, Tax Preparation, Invoicing, Client Intake, Bookkeeping, General Procedures, HR & Training, plus custom). All admin staff can browse, search, and read articles. Pinned articles appear first. Owner-only delete. Search filters by title, content, and category. AI assistant automatically references knowledge base articles when answering related questions, citing article titles. Table: `knowledge_articles`. API: CRUD at `/api/admin/knowledge-base`, search at `/api/admin/knowledge-base/search?q=`. Frontend: `/admin/knowledge-base`.
- **Client Portal AI Assistant**: Floating "Help" button in the client portal. Client-facing AI that knows the client's specific services, invoices, documents, and compliance needs. Uses the Knowledge Base to explain CC Trucking's processes. Shows dynamic suggested questions based on the client's active services (IFTA, DOT, tax prep, bookkeeping, etc.). Provides portal navigation links and escalation guidance. API: `POST /api/portal/ai-chat`. Component: `portal-ai-chat-widget.tsx`.
